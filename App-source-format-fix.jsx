...
import { supabase, CONFIG_READY, connex } from './supabaseClient.js'
import {
  Document, Packer, Paragraph, TextRun, AlignmentType, BorderStyle, HeadingLevel, convertInchesToTwip,
} from 'docx'

export const APP_VERSION = 'v2026:08:20-00:21'
...
// A promoted inbox row stores its source as "glassdoor — <full tracking URL>".
// The raw URL is unbreakable text that overflowed the card and pushed the
// status badge off screen, so lists show the readable name only. Nothing is
// discarded: the URL is still in the record and is linked in the detail view.
function splitSource(src) {
  if (!src) return { name: '', url: '' }
  const m = String(src).match(/^([\s\S]*?)\s+—\s+(https?:\/\/\S+)\s*$/)
  if (m) return { name: m[1].trim(), url: m[2] }
  const bare = String(src).trim()
  if (/^https?:\/\//i.test(bare)) {
    try { return { name: new URL(bare).hostname.replace(/^www\./, ''), url: bare } } catch { return { name: bare, url: '' } }
  }
        ...
        <div key={a.id} className="list-item" onClick={() => setOpenId(a.id)} style={{ cursor: 'pointer' }}>
          <div className="hstack">
            <div>
              ...
              <div className="muted small">{a.company}{splitSource(a.source).name ? ` · via ${splitSource(a.source).name}` : ''}</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginTop: 10 }}>
        ...
        <div className="muted">
          {a.company}{splitSource(a.source).name ? ` · via ${splitSource(a.source).name}` : ''}
          {splitSource(a.source).url && (
            <> · <a href={splitSource(a.source).url} target="_blank" rel="noreferrer">original posting ↗</a></>
          )}
        </div>
...
// ---------- Connex network integration -------------------------------------
// Live, read-only link to the user's Connex contacts (a separate Supabase
// project) so applications can surface who the user knows at the target company
// and, when the user selects contacts, feed those REAL relationships into the
// tailored CV / cover letter. Matching is by company/organization name only.

// Normalize a company/organization name for comparison: lowercase, strip
// punctuation, and drop common corporate suffixes and region words so
// "Globex K.K." matches "Globex" and "Globex Japan".
const COMPANY_STOPWORDS = new Set([
  'kk','k','inc','incorporated','ltd','limited','co','corp','corporation','company',
  'gmbh','pte','llc','plc','sa','ag','nv','bv','holdings','holding','group',
  'japan','asia','pacific','apac','emea','international','intl','global','the',
])
function normalizeCompany(v) {
  if (!v) return ''
  const cleaned = String(v).toLowerCase().replace(/[^a-z0-9\s]/g, ' ')
  return cleaned.split(/\s+/).filter(t => t && !COMPANY_STOPWORDS.has(t)).join(' ').trim()
}
// Match Connex cards to a company. Exact normalized match, or a contains match
// when the shorter normalized name is at least 4 chars (guards tiny tokens).
function matchContacts(cards, company) {
  const target = normalizeCompany(company)
  if (!target) return []
  return (cards || []).filter(c => {
    const cc = normalizeCompany(c.company)
    if (!cc) return false
    if (cc === target) return true
    const [short, long] = cc.length <= target.length ? [cc, target] : [target, cc]
    return short.length >= 4 && long.includes(short)
  })
}
// Fetch the user's Connex contacts (RLS scopes to the signed-in Connex user).
// Only the fields needed for matching and for the AI - never emails/phones.
async function fetchConnexContacts() {
  const { data, error } = await connex
    .from('cards')
    .select('id,full_name,job_title,department,company,notes,tags')
  if (error) throw error
  return data || []
}
// Track the Connex auth session. undefined = still loading, null = not connected.
function useConnex() {
  const [connexUser, setConnexUser] = useState(undefined)
  useEffect(() => {
    let alive = true
    connex.auth.getSession().then(({ data }) => { if (alive) setConnexUser(data.session?.user || null) })
    const { data: sub } = connex.auth.onAuthStateChange((_e, sess) => { if (alive) setConnexUser(sess?.user || null) })
    return () => { alive = false; sub.subscription.unsubscribe() }
  }, [])
  return connexUser
}
// Settings card: connect / disconnect the Connex account (a separate login).
function ConnexSettings({ notify }) {
  const connexUser = useConnex()
  const [email, setEmail] = useState('')
  const [pw, setPw] = useState('')
  const [busy, setBusy] = useState(false)
  ...
  useEffect(() => {
    if (!connexUser) { setCount(null); return }
    let alive = true
    connex.from('cards').select('id', { count: 'exact', head: true })
      .then(({ count }) => { if (alive) setCount(count ?? null) })
    return () => { alive = false }
  }, [connexUser])

  const signIn = async () => {
    setBusy(true)
    const { error } = await connex.auth.signInWithPassword({ email: email.trim(), password: pw })
    setBusy(false)
    if (error) notify('Connex sign-in failed: ' + error.message)
    else { setPw(''); notify('Connex connected.') }
  }
  const signOut = async () => { await connex.auth.signOut(); notify('Connex disconnected.') }

  return (
    <div className="card">
      <h2>Connex network</h2>
      {connexUser === undefined ? (
        <p className="muted small">Checking Connex connection...</p>
      ) : connexUser ? (
        <>
          <p className="muted small">
            Connected as {connexUser.email}{count != null ? ` - ${count} contacts available` : ''}. When you create an
            application, HiyakuAI surfaces the people you know at that company so you can feature them in the CV and cover letter.
          </p>
          <button className="ghost" onClick={signOut}>Disconnect Connex</button>
        </>
      ) : (
        <>
          <p className="muted small">
            Sign in to your Connex account to surface contacts by company for job applications. This is a separate login
            from HiyakuAI. Your contacts are read live and stay in Connex; only the contacts you explicitly select are ever
            sent to the AI.
          </p>
          <label>Connex email</label>
          <input value={email} onChange={e => setEmail(e.target.value)} autoComplete="off" />
          <label>Connex password</label>
          <input type="password" value={pw} onChange={e => setPw(e.target.value)} autoComplete="off" />
          <button style={{ marginTop: 8 }} onClick={signIn} disabled={busy || !email.trim() || !pw}>
            {busy && <span className="spin" />}Connect Connex
          </button>
        </>
      )}
    </div>
...
function NewApplication({ session, notify, done }) {
  ...
  // Connex network (live contacts at the target company)
  const connexUser = useConnex()
  ...
  // Load Connex contacts once when connected; recompute matches as company changes.
  useEffect(() => {
    if (!connexUser) { setContacts([]); setContactsLoaded(false); setSelectedIds(new Set()); return }
    let alive = true
    fetchConnexContacts()
      .then(cs => { if (alive) { setContacts(cs); setContactsLoaded(true) } })
      .catch(e => { if (alive) { setContactsLoaded(true); notify('Could not load Connex contacts: ' + e.message) } })
    return () => { alive = false }
  }, [connexUser])

  const matchedContacts = useMemo(() => matchContacts(contacts, company), [contacts, company])
  // Drop selections that no longer match the current company.
  useEffect(() => {
    setSelectedIds(prev => {
      const ok = new Set(matchedContacts.map(c => c.id))
        ...
        try {
          ...
          const { data, error } = await supabase.functions.invoke('hiyaku-generate', {
            body: {
              ...
              network: matchedContacts.filter(c => selectedIds.has(c.id)).map(c => ({
                name: c.full_name || '', title: c.job_title || '', department: c.department || '',
                company: c.company || '', note: c.notes || '', tags: (c.tags || []).join(', '),
              })),
            },
          })
          if (error) throw new Error(error.message)
  ...
  return (
    <div>
      ...
      {company && connexUser === null && (
        <div className="card">
          <p className="muted small">Connect your Connex account in Settings to surface people you know at {company} for this application.</p>
        </div>
      )}
      {company && connexUser && (
        <div className="card">
          <h3>Your network at {company}</h3>
          {!contactsLoaded ? (
            <p className="muted"><span className="spin" />Loading your Connex contacts...</p>
          ) : matchedContacts.length === 0 ? (
            <p className="muted small">No Connex contacts found at "{company}". Matching is by company/organization name.</p>
          ) : (
            <>
              <p className="muted small">
                Select the contacts you want the AI to reference in the CV and cover letter. Only the contacts you tick are
                sent, and each is represented exactly as your Connex data states it - never invented or upgraded.
              </p>
              {matchedContacts.map(c => (
                <label key={c.id} className="list-item" style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer' }}>
                  <input type="checkbox" checked={selectedIds.has(c.id)} onChange={() => toggleContact(c.id)} style={{ marginTop: 4 }} />
                  <div>
                    <strong>{c.full_name || '(no name)'}</strong>
                    <div className="muted small">{[c.job_title, c.department, c.company].filter(Boolean).join(' · ') || 'No title recorded'}</div>
                    {c.notes && <div className="muted small">Note: {c.notes}</div>}
                  </div>
                </label>
              ))}
              <p className="muted small" style={{ marginTop: 6 }}>
                {selectedIds.size} selected. These flow into "Generate tailored CV + cover letter" below.
              </p>
            </>
          )}
        </div>
      ...
      <div className="card">
        ...
        <p className="muted small">Generation uses only your Library facts — the AI is instructed to never invent claims. Any Connex contacts you selected above are referenced exactly as recorded, never upgraded.</p>
      </div>
